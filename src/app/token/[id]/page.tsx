...
import { formatPrice, formatPercentage, formatMarketCap, getRiskColor, getRiskLabel } from '@/lib/utils';
import { 
  TrendingUp, 
  TrendingDown, 
  Heart, 
  ...
  const loadTokenDetails = async () => {
    try {
      ...
      const mockPriceData = Array.from({ length: 30 }, (_, i) => ({
        ...
        price: tokenData.market_data.current_price.usd * (1 + (Math.random() - 0.5) * 0.2),
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-4">
                ...
                <div>
                  ...
                  <p className="text-muted-foreground text-lg uppercase">{token.symbol}</p>
                </div>
              </div>
              <div className="flex items-center space-x-4">
                <div className="text-right">
                  <p className="text-3xl font-bold">{formatPrice(token.market_data.current_price.usd)}</p>
                  <div className={`flex items-center justify-end space-x-1 ${isPositive ? 'text-green-400' : 'text-red-400'}`}>
                    {isPositive ? <TrendingUp className="w-4 h-4" /> : <TrendingDown className="w-4 h-4" />}
                    <span className="font-medium">{formatPercentage(token.market_data.price_change_percentage_24h)}</span>
                  </div>
                </div>
                <div className="flex items-center space-x-2">
                  ...
                  >
                    <Heart className={`w-4 h-4 ${isTokenFavorite ? 'fill-current' : ''}`} />
                  </Button>
                ...
                <Card className="p-6">
                  ...
                  <div className="space-y-4">
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium">{formatMarketCap(token.market_data.market_cap.usd)}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-muted-foreground">24h Volume</span>
                      <span className="font-medium">{formatMarketCap(token.market_data.total_volume.usd)}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-muted-foreground">Circulating Supply</span>
                      <span className="font-medium">{token.market_data.circulating_supply?.toLocaleString()}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-muted-foreground">All-Time High</span>
                      <span className="font-medium">{formatPrice(token.market_data.ath.usd)}</span>
                    </div>
                  </div>
                </Card>
              </div>
              ...
              {aiAnalysis ? (
                    ...
                    <CardContent>
                      <div className="mb-6">
                        <div className="flex items-center justify-between mb-2">
                          ...
                          <span className={`text-2xl font-bold ${getRiskColor(aiAnalysis.risk_score)}`}>
                      ...
                      <p className="text-muted-foreground mb-4">{aiAnalysis.summary}</p>
                ...
                <Card className="p-12 text-center">
                  ...
                  <p className="text-muted-foreground mb-4">
                    Click "Analyze with AI" to generate a comprehensive analysis of this token.
                  </p>
                  <Button onClick={handleAnalyze} loading={isAnalyzing}>
                    <Brain className="w-4 h-4 mr-2" />
                    Analyze with AI
          ...
          {activeTab === 'charts' && (
            <Card className="p-6">
              ...
              <div className="h-96 bg-muted/20 rounded-lg flex items-center justify-center">
                <p className="text-muted-foreground">TradingView widget would go here</p>
              </div>
            </Card>
          )}
            ...
            <Card className="p-6">
              ...
              <div className="prose prose-invert max-w-none">
                <p className="text-muted-foreground">
                  {token.description?.en || 'No description available for this token.'}
                </p>
                {token.links && (
                  <div className="mt-6">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {token.links.homepage[0] && (
                        <Button variant="outline" size="sm" asChild>
                          <a href={token.links.homepage[0]} target="_blank" rel="noopener noreferrer">
                      ...
                      {token.links.twitter_screen_name && (
                        <Button variant="outline" size="sm" asChild>
                          <a href={`https://twitter.com/${token.links.twitter_screen_name}`} target="_blank" rel="noopener noreferrer">
